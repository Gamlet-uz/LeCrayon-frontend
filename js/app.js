...
const app = {
  ...
  formatPhones: (phoneStr) => {
    ...
    `;
    for(let i = 1; i < phones.length; i++) html += `<strong>${phones[i]}</strong>`;
    html += `</div>`;
    return `<div style="width:100%;">${html}</div>`;
  },

  // KIRILL -> LOTIN O'GIRUVCHI (QIDIRUV UCHUN)
  ...
  setupAuthListeners: () => {
    ...
    const bioLoginBtn = document.getElementById('btn-faceid-login');
    if (bioLoginBtn) {
      bioLoginBtn.addEventListener('click', () => {
        if (!tg.isVersionAtLeast('7.2')) return tg.showAlert("Telegram versiyangiz eskirgan (7.2 dan yuqori bo'lishi kerak). Iltimos yangilang.");
        ...
        setTimeout(() => {
          ...
          tg.BiometricManager.authenticate({ reason: "Tizimga kirish uchun barmoq izingizni tasdiqlang" }, async (success, token) => {
            if (success && token) {
              ...
              if (parts.length !== 2) return tg.showAlert("Biometrika ma'lumotlari xato saqlangan, iltimos qaytadan ulab ko'ring.");
              
              try {
                app.toggleLoader(true);
                const res = await API.login(parts[0], parts[1], telegramId);
                app.toggleLoader(false);
              ...
              } catch (err) { app.toggleLoader(false); tg.showAlert("Tarmoq xatosi: " + err.message); }
            } else if (!success) {
              tg.showAlert("Barmoq izini tasdiqlash bekor qilindi yoki xato.");
            }
          });
        }, 300); 
      });
    ...
    // BARMOQ IZINI ULASH
    const registerBioBtn = document.getElementById('btn-register-faceid');
    if(registerBioBtn) {
          ...
          tg.BiometricManager.requestAccess({ reason: "Parolsiz kirish uchun barmoq izini faollashtiring" }, (granted) => {
            if (granted) {
              ...
              tg.BiometricManager.updateBiometricToken(tokenToSave, (updated) => {
                if(updated) tg.showAlert("✅ Barmoq izi muvaffaqiyatli ulandi! Endi login oynasidan parolsiz kirishingiz mumkin.");
              });
            } else { tg.showAlert("Barmoq iziga ruxsat berilmadi."); }
          });
        }, 300);
      });
    }
  ...
  applyCrop: () => {
    ...
    // Rasm sifati pasaytirildi (kenglik 300px va sifat 0.5 - tez yuklanishi uchun)
    const canvas = app.cropper.getCroppedCanvas({ width: 300, height: 300 });
  ...
  setupTeacherListeners: () => {
    document.getElementById('form-t-add-class').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        app.toggleLoader(false);
        if(res.success) { tg.showAlert("Sinf yaratildi!"); app.goBack(); app.loadTeacherClasses(); }
      } catch(err) { app.toggleLoader(false); tg.showAlert(err.message); }
    });
    ...
    document.getElementById('form-t-edit-class').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        app.toggleLoader(false);
        if(res.success) { 
          app.activeClassName = name; tg.showAlert("Sinf nomi yangilandi!"); 
          app.goBack(); document.getElementById('my-class-title-text').innerText = `Sinf: ${name}`; 
        }
      } catch(err) { app.toggleLoader(false); tg.showAlert(err.message); }
    });
    ...
    // EXCEL YUKLASH (USTOZ UCHUN - SHU SINFGA BOG'LIQ)
      ...
      reader.onload = async (ev) => {
        try {
          ...
          for(let i = 1; i < rows.length; i++) {
            ...
            // 10 ta Sertifikatni o'qish mantig'i 
            let certs = [];
            // Exceldagi ustunlar tartibi: Ism(0), Tug'ilgan sana(1), Doimiy(2), Yotoqxona(3), Ota-ona tel(4), Yotoqxona tel(5), Cert1(6-8)...
            for(let j=0; j<10; j++) {
              let base = 6 + (j*3);
            ...
            studentsList.push({
              ...
              permanent_address: String(r[2] || ''),
              dormitory_address: String(r[3] || ''),
              parent_phone: String(r[4] || ''),
              dormitory_phone: String(r[5] || ''),
              certificates: certs
            });
          }
          if(studentsList.length === 0) { app.toggleLoader(false); return tg.showAlert("Excel faylda o'qish uchun ma'lumot topilmadi!"); }
        ...
        } catch (error) { app.toggleLoader(false); tg.showAlert("Faylni o'qishda xatolik: " + error.message); }
      };
  ...
  switchStudentTab: (tab) => {
    document.getElementById('tab-manual').classList.remove('active'); document.getElementById('tab-excel').classList.remove('active');
    document.getElementById('form-t-add-student-manual').classList.add('hidden'); document.getElementById('form-t-add-student-excel').classList.add('hidden');
    document.getElementById(`tab-${tab}`).classList.add('active'); document.getElementById(`form-t-add-student-${tab}`).classList.remove('hidden');
  },
  ...
  loadTeacherStudents: async () => {
    try {
      ...
      if(res.success) {
        ...
        c.innerHTML = '';
        if(res.data.length === 0) return c.innerHTML = "<p style='text-align:center;'>Ushbu sinfda o'quvchilar mavjud emas.</p>"; 
    ...
    } catch(err) { app.toggleLoader(false); tg.showAlert("Yuklashda xatolik: " + err.message); }
  },
  ...
  openEditStudent: (id) => {
    ...
    document.getElementById('edit-st-birth').value = st.birth_date || ''; 
  ...
  deleteStudent: (id) => {
    tg.showConfirm("Ushbu o'quvchini o'chirib yubormoqchimisiz?", async (confirm) => {
      if(confirm) {
        try {
          app.toggleLoader(true); const res = await API.deleteStudent(id); app.toggleLoader(false);
          if(res.success) { tg.showAlert("O'chirildi!"); app.loadTeacherStudents(); }
  ...
  // =====================================
  // DAVOMAT QILISH (VA TAHRIRLASH UCHUN TEKSHIRUV)
  // =====================================
    ...
    try {
      ...
      if(res.success) {
        ...
        if(res.data.length === 0) { 
          ...
          const isSubmitted = attRes.success && attRes.isSubmitted;
  ...
  adminLoadTeachers: async () => {
    try {
      app.toggleLoader(true); const res = await API.getTeachers(); app.toggleLoader(false);
    ...
    app.chartInstance = new Chart(ctx, {
      ...
      data: { 
        ...
        datasets: [{ 
          ...
          backgroundColor: ['#34c759', '#ff9500', '#ff3b30', '#007aff'], borderWidth: 2, hoverOffset: 5 
        }] 
      },
      options: { 
        responsive: true, maintainAspectRatio: false, 
        plugins: { legend: { position: 'bottom' }, title: { display: true, text: `Jami o'quvchi: ${stats.total}` } }, 
        onClick: (event, elements) => { 
          if (elements.length > 0) app.filterMonitoringList(['keldi', 'sababli', 'sababsiz', 'kech_keldi'][elements[0].index]); 
        } 
      }
    });
  },
  ...
  filterMonitoringList: (status) => {
    ...
    const title = document.getElementById('monitoring-list-title'); title.style.display = 'block';
    if (status === 'keldi') return title.innerText = "Kelganlar ro'yxati bu yerda ko'rsatilmaydi.";
  ...
  searchStudents: (query) => {
    ...
    app.searchTimeout = setTimeout(async () => {
      if(!latinQuery.trim()) return document.getElementById('admin-search-results').innerHTML = '';
      
      try {
        app.toggleLoader(true); const res = await API.searchStudents(latinQuery); app.toggleLoader(false);
        if(res.success) {
          ...
          if(res.data.length === 0) c.innerHTML = '<p style="text-align:center;">Topilmadi.</p>';
              ...
              c.innerHTML += `
                <div class="student-card" style="cursor:pointer;" onclick="app.showStudentDetails('${st.id}', 'search')">
                  <img src="${st.photo_url}" alt="">
                  <div class="info"><h4>${st.full_name}</h4><p>Sinf: ${st.class_name}</p></div>
                </div>`; 
            });
        }
  ...
  openAdminSettings: async () => {
    try {
      ...
      if (resSettings.success && resSettings.time) document.getElementById('reminder-time').value = resSettings.time;
      if (resClasses.success) { 
        const s1 = document.getElementById('delete-class-select'); s1.innerHTML = '<option value="">Sinfni tanlang...</option>'; 
        resClasses.data.forEach(c => s1.innerHTML += `<option value="${c.id}">${c.class_name}</option>`); 
      }
      if (resTeachers.success) { 
        const s2 = document.getElementById('delete-teacher-select'); s2.innerHTML = '<option value="">O\'qituvchini tanlang...</option>'; 
  ...
  setupAdminListeners: () => {
    document.getElementById('form-broadcast').addEventListener('submit', async (e) => { 
      e.preventDefault(); 
      try { 
        app.toggleLoader(true); await API.sendBroadcast(document.getElementById('broadcast-message').value); app.toggleLoader(false); 
        tg.showAlert(`E'lon yuborildi!`); document.getElementById('form-broadcast').reset(); 
      } catch(err) { app.toggleLoader(false); } 
    });
      ...
      reader.onload = async (ev) => {
        try {
          ...
          for(let i = 1; i < rows.length; i++) {
             ...
             // Admin shablon format (0: Ism, 1: Tug'ilgan sana, 2: Sinf nomi, 3: Doimiy, 4: Yotqx, 5: Ota-ona tel, 6: Yotqx tel, 7+: Certs)
             for(let j=0; j<10; j++) {
                 let base = 7 + (j*3);
                 if(r[base]) certs.push({ name: String(r[base]||''), level: String(r[base+1]||''), percent: String(r[base+2]||'') });
             }
          ...
          const res = await API.adminGlobalBulkCreateStudents(studentsList);
          app.toggleLoader(false);

          if (res.success) {
            tg.showAlert(`${res.count} nafar o'quvchi muvaffaqiyatli yuklandi!`);
            document.getElementById('form-admin-bulk-students').reset();
          } else { tg.showAlert(res.error || "Xatolik yuz berdi"); }
        } catch (error) { app.toggleLoader(false); tg.showAlert("Faylni o'qishda xatolik: " + error.message); }
      };
      reader.readAsArrayBuffer(file);
    ...
    document.getElementById('form-settings').addEventListener('submit', async (e) => { 
      e.preventDefault(); 
      try { 
        app.toggleLoader(true); await API.saveSettings(document.getElementById('reminder-time').value); app.toggleLoader(false); 
        tg.showAlert("Sozlamalar saqlandi!"); 
      } catch(err) { app.toggleLoader(false); } 
    });

    document.getElementById('form-delete-class').addEventListener('submit', async (e) => { 
      e.preventDefault(); const cId = document.getElementById('delete-class-select').value; if(!cId) return; 
      tg.showConfirm("O'chirasizmi?", async (conf) => { 
        if(conf) { 
          try { app.toggleLoader(true); await API.deleteClass(cId); app.toggleLoader(false); tg.showAlert("Sinf o'chirildi"); app.openAdminSettings(); } 
          catch(err) { app.toggleLoader(false); } 
        } 
      }); 
    });
    ...
    document.getElementById('form-delete-teacher').addEventListener('submit', async (e) => { 
      e.preventDefault(); const tId = document.getElementById('delete-teacher-select').value; if(!tId) return; 
      tg.showConfirm("O'chirasizmi?", async (conf) => { 
        if(conf) { 
          try { app.toggleLoader(true); await API.deleteTeacher(tId); app.toggleLoader(false); tg.showAlert("O'qituvchi o'chirildi"); app.openAdminSettings(); } 
          catch(err) { app.toggleLoader(false); } 
        } 
      }); 
    });
    ...
    document.getElementById('form-edit-profile').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if(app.currentUser.role === 'teacher') {
          ...
          if(photoBlob) { app.toggleLoader(true); data.photo_url = await API.uploadImage(photoBlob); app.toggleLoader(false); }
        }
        app.toggleLoader(true); const res = await API.updateProfile(app.currentUser.id, data); app.toggleLoader(false);
        if(res.success) { app.currentUser = res.user; localStorage.setItem('leCrayonUser', JSON.stringify(res.user)); tg.showAlert("Ma'lumotlar yangilandi!"); app.goBack(); }
      } catch(err) { app.toggleLoader(false); tg.showAlert(err.message); }
    });
  },
  ...
  showProfileEdit: () => {
    ...
    if(app.currentUser.role === 'teacher') {
      ...
      document.getElementById('edit-photo-box').classList.add('hidden'); document.getElementById('edit-address').parentElement.classList.add('hidden'); document.getElementById('edit-phone').parentElement.classList.add('hidden');
    }
  ...
  gatherCertificates: (containerId) => {
    ...
    document.getElementById(containerId).querySelectorAll('.certificate-group').forEach(g => { 
      certs.push({ name: g.querySelector('.cert-name').value, level: g.querySelector('.cert-level').value, percent: g.querySelector('.cert-percent').value }); 
    }); 
    return certs;
  },
          ...
          res.data.forEach(rec => { 
            c.innerHTML += `
              <div class="diary-item ${rec.status}">
                <div class="diary-date-status"><span><i class="fa-regular fa-calendar"></i> ${rec.date}</span><span style="text-transform:uppercase;">${rec.status.replace('_', ' ')}</span></div>
  ...
  closeDiaryModal: () => { document.getElementById('diary-modal').classList.add('hidden'); },
  ...
  showStudentDetails: (id, context) => {
    ...
    document.getElementById('modal-body').innerHTML = `
      ...
      <div class="modal-data-row" style="align-items:flex-start;"><span>Ota-ona telfoni</span> ${app.formatPhones(st.parent_phone)}</div>
      <div class="modal-data-row" style="align-items:flex-start;"><span>Yotoqxona telfoni</span> ${app.formatPhones(st.dormitory_phone)}</div>
